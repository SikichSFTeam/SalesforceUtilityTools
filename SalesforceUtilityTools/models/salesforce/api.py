...
class SFType(object):
    ...
    def cache(self, fieldList = set(), headers=None):
        """Does a thing to cache an object for usage
        Arguments:
        * headers -- a dict with additional request headers.
        """
        desc = self.describe(headers)
        
        dir_path = os.path.dirname(os.path.realpath(__file__))

        with open(dir_path + '/objects/' + self.name.lower() + '.py', 'w') as file:
            try:
                file.write('from connectors.salesforce import Salesforce \n')
                file.write('\n')
                file.write('class ' + self.name + '(SfObject): \n')
                file.write('  def __init__(self): \n')
                file.write('    self._name = \'' + self.name + '\' \n')
                file.write('\n')
                file.write('  fields = { \n')

                for field in desc['fields']:
                    if field['name'] in fieldList or (len(fieldList) == 0 and (field['createable'] or field['updateable'])):
                        datatype = 'sfData()'

                        if field['type'] in ['double', 'currency', 'percent', 'int']:
                            datatype = 'sfNumeric(' + str(field['precision']) + ', ' + str(field['scale']) + ')'
                        elif field['type'] in ['string', 'textarea', 'reference', 'picklist', 'multipicklist', 'phone', 'email', 'id', 'address', 'url', 'encryptedstring']:
                            datatype = 'sfText(' + str(field['length']) + ')'
                        ...
                        upd = 'Updateable' if field['updateable'] else ''
                        crt = 'Creatable' if field['createable'] else ''

                        file.write('    \'' + field['name'] + '\': ' + datatype + ',  #' + field['type'] + ' ' + crt + ' ' + upd +'\n')
                        if len(field['picklistValues']) > 0:
                            file.write('      # PickList Values:' + '\n')
                            for pl in field['picklistValues']:
                                if pl['active']:
                                    file.write('        # ' + pl['value'] + '\n')

                        if len(field['referenceTo']) > 0:
                            file.write('      # Reference To:' + '\n')
